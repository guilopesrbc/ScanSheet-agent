...
class TestScanSheetAgent(unittest.TestCase):
    ...
    @patch('scansheet_agent.agent.AIMessageModel.model_validate')
    def test_validate_model_response_success(self, mock_model_validate):
        ...
        # Call the method
        result = self.agent.validate_model_response(mock_response)
    ...
    def test_validate_model_response_invalid_json(self, mock_model_validate):
        ...
        with self.assertRaises(Exception):
            self.agent.validate_model_response(mock_response)
    ...
    def test_invoke_model(self):
        ...
        # Call the method
        result = self.agent.invoke_model(mock_chain, inputs)

        # Assertions
        mock_chain.invoke.assert_called_once_with(inputs)
        self.assertEqual(result, "mock_response")

    @patch('scansheet_agent.agent.ScanSheetAgent.validate_model_response')
    def test_build_chain(self, mock_validate):
        ...
        # Call the method
        chain = self.agent.build_chain(mock_prompt)

        # We can't easily test the chain itself, but we can ensure it was created
        self.assertIsNotNone(chain)

    @patch('scansheet_agent.agent.ScanSheetAgent.build_chain')
    @patch('scansheet_agent.agent.ScanSheetAgent.invoke_model')
